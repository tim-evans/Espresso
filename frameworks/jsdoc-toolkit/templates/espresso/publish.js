/*global JSDOC Link SYS defined IO Showdown*/
...
function removeExtraSpaces(block) {
  var lines = String(block).split('\n'),
      minSpaces, i, j, len = lines.length;
  if (/\s/.test(lines[0][0])) {
    i = 0;
  } else {
    i = 1;
  ...
  for (i; i < len; i++) {
    ...
    while (lines[i][j++] === ' ') {}
    minSpaces = minSpaces < j ? minSpaces: j;
  }

  if (!/^\s/.test(lines[0][0]) && minSpaces - 1 > 0) {
    var spaces = '';
  ...
  for (i = 0; i < len; i++) {
    lines[i] = lines[i].slice(minSpaces - 1);
  }
  return lines.join('\n');
}
...
function markdown(block) {
  ...
  block = removeExtraSpaces(String(block));
...
//
// Original Markdown Copyright (c) 2004-2005 John Grube
	...
	*/
	...
	text = text.replace(/(?:\n\n|^)((?:(?:[ ]{4}|\t).*\n+)+)(\n*[ ]{0,3}[^ \t\n]|(?=~0))/g,
		function(wholeMatch,m1,m2) {
			...
			codeblock = code(_UnescapeSpecialChars(codeblock));
